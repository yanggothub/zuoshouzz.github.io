...
#	Version: 2.0.38
#	Author: zuoshou
#	Blog: https://zuoshouzz.github.io
#=================================================
...
Set_config_password(){
	...
	read -e -p "(默认: zuoshouzz.github.io):" ssr_password
	[[ -z "${ssr_password}" ]] && ssr_password="zuoshouzz.github.io"
...
Update_Shell(){
	sh_new_ver=$(wget --no-check-certificate -qO- -t1 -T3 "https://raw.githubusercontent.com/zuoshouzz/zuoshouzz.github.io/master/doc/ssr.sh"|grep 'sh_ver="'|awk -F "=" '{print $NF}'|sed 's/\"//g'|head -1) && sh_new_type="github"
	[[ -z ${sh_new_ver} ]] && echo -e "${Error} 无法链接到 Github !" && exit 0
	if [[ -e "/etc/init.d/ssr" ]]; then
		rm -rf /etc/init.d/ssr
		Service_SSR
	fi
	wget -N --no-check-certificate "https://raw.githubusercontent.com/zuoshouzz/zuoshouzz.github.io/master/doc/ssr.sh" && chmod +x ssr.sh
